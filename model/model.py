...
class SlotVQA(nn.Module): 
    ...
    def __init__(self, 
        ...
        self.transf2vocab = nn.Sequential(
                nn.Linear(self.transf_dim, vocab), 
                nn.Softmax(dim=-1)) 
    ...
    def forward(self, 
        ...
        if captioning: 
            ...
            assert caption != None, "Caption missing" 

            img_att, img_emb, img_slots = self.img_enc(inp=img) 
            img_slots = self.linear_img(img_slots) 
            sub_mask = torch.triu(torch.ones((tgt.shape[1], tgt.shape[1]), device=device)*float('-inf'), diagonal=1)

            out = self.transf_dec(tgt=caption, memory=img_slots,mask=submask) 
            out = self.transf2vocab(out)  
